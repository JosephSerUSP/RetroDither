...
export class App {
                ...
                number.addEventListener('blur', () => {
                    if (number.value === '' || isNaN(parseInt(number.value))) {
                        number.value = this.state[key];
                         ...
                         // Also force clamp update on blur to be safe
    ...
    setupViewportInteraction() {
        ...
        // Zoom Buttons
        const modifyZoom = (delta) => {
            if (this.state.zoomSnap) {
                let newZoom = Math.round(this.state.zoom + delta);
                newZoom = Math.max(1, newZoom);
                this.state.zoom = newZoom;
            } else {
                this.state.zoom *= (delta > 0 ? 1.1 : 0.9);
            }
            this.updateTransform();
            this.updateZoomStatus();
        };

        if(this.ui.els.btnZoomIn) this.ui.els.btnZoomIn.addEventListener('click', () => modifyZoom(1));
        if(this.ui.els.btnZoomOut) this.ui.els.btnZoomOut.addEventListener('click', () => modifyZoom(-1));
    }
