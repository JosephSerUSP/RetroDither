...
export class App {
    ...
    initListeners() {
        ...
        // Prevent default on document to avoid opening file in browser
        ...
        vp.addEventListener('dragleave', (e) => {
            e.preventDefault();
            // Use a timer to prevent flicker when entering children
    ...
    setupViewportInteraction() {
        ...
        // Zoom Buttons
        if(this.ui.els.btnZoomIn) {
            this.ui.els.btnZoomIn.addEventListener('click', () => {
                if (this.state.zoomSnap) {
                    this.state.zoom = Math.round(this.state.zoom + 1);
                } else {
        ...
        if(this.ui.els.btnZoomOut) {
            this.ui.els.btnZoomOut.addEventListener('click', () => {
                if (this.state.zoomSnap) {
                    this.state.zoom = Math.max(1, Math.round(this.state.zoom - 1));
                } else {
    ...
    triggerProcess() {
        ...
        // If using a custom palette, we must pass the actual color data to the worker
        // because the worker has its own isolated PALETTES object.
        if (this.state.palette.startsWith('custom_') && PALETTES[this.state.palette]) {
            options.customPaletteData = PALETTES[this.state.palette];
        }
