    ...
    initListeners() {
        ...
        this.ui.els.selPalette.addEventListener('change', (e) => {
            if (e.target.value === 'import_custom') {
                e.target.value = this.state.palette;
        ...
        this.ui.els.btnZoomIn.addEventListener('click', (e) => {
            e.stopPropagation();
            this.state.zoom = this.state.zoomSnap ? Math.round(this.state.zoom + 1) : this.state.zoom * 1.1;
            this.updateTransform();
            this.updateZoomStatus();
        });

        this.ui.els.btnZoomOut.addEventListener('click', (e) => {
            e.stopPropagation();
            this.state.zoom = this.state.zoomSnap ? Math.max(1, Math.round(this.state.zoom - 1)) : Math.max(0.1, this.state.zoom / 1.1);
            this.updateTransform();
            this.updateZoomStatus();
        });
