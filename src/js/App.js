...
export class App {
    ...
    setupZoomButtons() {
        if(this.ui.els.btnZoomIn) {
            this.ui.els.btnZoomIn.addEventListener('click', () => {
                if(this.state.zoomSnap) {
                    this.state.zoom = Math.floor(this.state.zoom) + 1;
                } else {
        ...
        if(this.ui.els.btnZoomOut) {
            this.ui.els.btnZoomOut.addEventListener('click', () => {
                if(this.state.zoomSnap) {
                    this.state.zoom = Math.max(1, Math.ceil(this.state.zoom) - 1);
                } else {
                    this.state.zoom /= 1.25;
    ...
    addCustomPalette(name, colors) {
        ...
        PALETTES[id] = colors;
        this.state.customPalettes[id] = colors; // Store in state for worker
        ...
        // Update worker with new palette
        this.processor.worker.postMessage({
            type: 'addPalette',
            id: id,
            colors: colors
        });
